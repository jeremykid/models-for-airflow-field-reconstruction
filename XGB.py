...
kf = KFold(n_splits=5, shuffle=True, random_state=20220)
xgb = XGBRegressor(max_depth=10, n_estimators=2000, random_state=20220)
result_xgb_sk = cross_validate(xgb, X, y, cv=kf, scoring='neg_root_mean_squared_error', return_train_score = True, verbose=True, n_jobs=-1)
...
xgb.fit(X, y) # train model with all data
...
# Test Step
test_inputs = [[1,1],[1,0],[1,2],[0,1],[2,1]]
df_results = []
...
df_final.to_csv('train_prediction.csv', index=False)
